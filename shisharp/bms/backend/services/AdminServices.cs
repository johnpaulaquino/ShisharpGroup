...
namespace BrgyMs.backend.services {
    public class AdminServices : BaseServices {
        ...
        private readonly UserInfoValidation uservalidation = new UserInfoValidation();
            ...
            try {
                using (var reader = await _AdminRepository.GetUserAllInformation(UserId)) {
                    ...
                    while (reader.Read()) {
                        ...
                        if (string.Equals(reader.GetInt32(reader.GetOrdinal("status")).ToString(), "1")) {
                            status = true;
                        }

                        user.Add(new User(reader.GetString(reader.GetOrdinal("email")),

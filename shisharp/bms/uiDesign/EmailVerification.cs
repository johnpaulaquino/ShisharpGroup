        ...
        private async void btnSubmit_Click(object sender, EventArgs e) {
            try {
                if (isclicked) {
                    ...
                    Dictionary<string, string> data = await _Residentrepo.GetEmail(txtEmail.Text);
                    ...
                    await Task.Run(async () =>
                    {
                        string otp = _AuthUtils.GenerateOTP();
                        await _EmailServices.SendPlainEmail(
                      txtEmail.Text.Trim(), "Email Verification", $"This is your OTP. {otp}.");

                    });
                    _Utils.PutIdOnFile(data["userId"]);
                ...
                MessageBox.Show("OTP is sent to your email address.");
